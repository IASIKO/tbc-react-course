...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const authUser =
      await sql`SELECT * FROM auth_users where sub = ${profile.sub}`;
    if (!authUser.rows.length) {
      await sql`INSERT INTO auth_users (given_name, family_name, country, city, address, phone, email, sub, picture) VALUES (${given_name}, ${family_name}, ${country}, ${city}, ${address}, ${phone}, ${email}, ${sub}, ${picture});`;
    } else {
